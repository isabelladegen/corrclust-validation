...
def test_combines_all_correlation_results_for_index_and_all_reductions_into_one_df_for_clusters():
    ...
    ir = ImpactReductionOnInternalIndices(overall_ds_name=overall,
                                          ...
                                          data_completeness=DataCompleteness.irregular_p30, n_dropped=n_dropped,
                                          reduction_type=reduction_type, internal_measure=internal_measure,
    ...
    assert_that(n_dropped_17_vals.iloc[0], is_(0.822))
    assert_that(n_dropped_17_vals.iloc[1], is_(0.9))
...
def test_combines_all_correlation_results_for_index_and_all_reductions_into_one_df_for_segments():
    ...
    ir = ImpactReductionOnInternalIndices(overall_ds_name=overall,
                                          ...
                                          root_reduced_data_dir=root_reduced_data_dir,
                                          unreduced_data_dir=unreduced_data_dir,
                                          data_completeness=DataCompleteness.irregular_p90, n_dropped=n_dropped,
                                          reduction_type=reduction_type, internal_measure=internal_measure,
                                          distance_measure=distance_measure)
    ...
    n_dropped_75_vals = df[
        get_col_name_reduction_internal_corr(internal_measure=internal_measure, n_dropped=75,
                                             reduction_type=reduction_type)]
    assert_that(n_dropped_75_vals.iloc[0], is_(0.896))
    assert_that(n_dropped_75_vals.iloc[1], is_(0.921))
    ...
    assert_that(paired_t_test.shape, is_((4, 3)))
    results = paired_t_test[get_col_reductions_compared(0, 75, reduction_type)]
    assert_that(results.loc[StatsCols.p_value], is_(0.45))
    assert_that(results.loc[StatsCols.achieved_power], is_(0.115))
    assert_that(results.loc[StatsCols.effect_size], is_(0.14))
