...
def calculate_y_pred_from(labels_df: pd.DataFrame):
    """
    Calculates ye pred from the labels df. For this each pattern id occurs for each observation in the data
...
def calculate_cluster_centroids(labels_df: pd.DataFrame, data: np.array, round_to: int = 3):
    ...
    for pattern in patterns:
        ...
        labels_for_pattern = labels_df[labels_df[SyntheticDataSegmentCols.pattern_id] == pattern].reset_index()
        n_segments = labels_for_pattern.shape[0]

        # select all observations for this pattern
        pattern_data = None
        for seg in range(n_segments):
            start_idx = labels_for_pattern.loc[seg, SyntheticDataSegmentCols.start_idx]
            end_idx = labels_for_pattern.loc[seg, SyntheticDataSegmentCols.end_idx]
            length = labels_for_pattern.loc[seg, SyntheticDataSegmentCols.length]
