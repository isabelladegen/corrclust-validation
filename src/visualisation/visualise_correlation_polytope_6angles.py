...
def setup_single_subplot(ax, fontsize):
    ...
    ax.text(1.15, -0.05, -0.05, r'$a_{12}$', fontsize=fontsize, color='black', zorder=float('inf'))
    ax.text(0.1, 1.15, 0.05, r'$a_{13}$', fontsize=fontsize, color='black', zorder=float('inf'))
    ax.text(0.1, -0.05, 1.15, r'$a_{23}$', fontsize=fontsize, color='black', zorder=float('inf'))
...
def plot_polytope_4_views(resolution=60, alpha_front=0.6, alpha_back=0.2):
    """
    Plot correlation polytope from 6 different viewing angles with consistent coloring
    """
    fontsize = 18
